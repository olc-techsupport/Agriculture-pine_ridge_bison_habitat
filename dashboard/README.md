...
This portable command works from a cloned GitHub repository without assuming
where Conda is installed. It works without `conda activate` and bypasses
Streamlit's optional email prompt. Open `http://127.0.0.1:8501` and keep the
terminal open while using the dashboard.

GitHub does not execute Streamlit itself. A public, no-install dashboard must
be deployed to a Streamlit-compatible host and linked from the main README.
